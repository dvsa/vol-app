...
final class CurrentDiscsTest extends MockeryTestCase
{
    #[\PHPUnit\Framework\Attributes\DataProvider('dpMapFromResult')]
    public function testMapFromResult($apiData, $formData): void
    {
        $this->assertEquals($formData, CurrentDiscs::mapFromResult($apiData));
    }

    #[\PHPUnit\Framework\Attributes\DataProvider('dpMapFromForm')]
    public function testMapFromForm($apiData, $formData): void
    {
        $this->assertEquals($apiData, CurrentDiscs::mapFromForm($formData));
    }

    /**
     * @return \Iterator<(int | string), array<array<(array<(array<(int | string | null)> | string)> | int | string | null)>>>
     *
     ...
     */
    public static function dpMapFromResult(): \Iterator
    {
        yield 'case_01' => [
            'apiData' => [
                "id" => 5,
