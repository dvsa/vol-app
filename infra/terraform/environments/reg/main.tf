locals {
  ...
  task_iam_role_statements = [
    ...
    },
    {
      ...
      resources = [
        "arn:aws:sqs:eu-west-1:054614622558:DEVAPPREG-OLCS-PRI-CHGET-INSOLVENCY-DLQ",
        "arn:aws:sqs:eu-west-1:054614622558:DEVAPPREG-OLCS-PRI-CHGET-INSOLVENCY",
        "arn:aws:sqs:eu-west-1:054614622558:DEVAPPREG-OLCS-PRI-CHGET-DLQ",
        "arn:aws:sqs:eu-west-1:054614622558:DEVAPPREG-OLCS-PRI-CHGET"
      ]
    },
    {
      effect = "Allow"
    ...
    },
    {
      ...
      resources = [
        "arn:aws:rds:eu-west-1:054614622558:cluster:reg-aurora-olcsdb-cluster",
    ...
    },
    {
      ...
      resources = [
        ...
        "arn:aws:rds:eu-west-1:054614622558:cluster:olcs-anon-*",
        "arn:aws:rds:eu-west-1:054614622558:cluster:ni-extract-*",
        "arn:aws:rds:eu-west-1:054614622558:subgrp:DEVAPPREG-olcs-rds-*"
      ]
    },
    {
      effect = "Allow"
...
data "aws_security_group" "this" {
  ...
  name = "DEV/APP/REG-OLCS-PRI-${each.key}-SG"
}
...
data "aws_subnets" "this" {
  ...
  filter {
    name = "tag:Name"
    values = [
      "DEV/APP/REG-OLCS-PRI-${each.key}-1A",
      "DEV/APP/REG-OLCS-PRI-${each.key}-1B",
      "DEV/APP/REG-OLCS-PRI-${each.key}-1C"
    ]
  }
}
...
data "aws_secretsmanager_secret" "this" {
  ...
  name = "DEVAPPREG-BASE-SM-APPLICATION-${upper(each.key)}"
}

data "aws_secretsmanager_secret" "infra" {
  name = "DEVAPPREG-BASE-SM-INFRA"
}

data "aws_cognito_user_pools" "this" {
  name = "DVSA-DEVAPPREG-COGNITO-USERS"
}

data "aws_lb" "this" {
  for_each = setsubtract(local.legacy_service_names, ["RENDERER"])

  name = "DEVAPPREG-OLCS-PRI-${(each.key == "API" ? "SVCS" : each.key)}-ALB"
}
...
module "service" {
  ...
  elasticache_url = "tcp://cache.reg.olcs.dev-dvsacloud.uk:6379"
