...
| <a name="input_jobs"></a> [jobs](#input\_jobs) | The application command to run | <pre>map(object({<br>    job_config = list(object({<br>      command            = string<br>      image              = string<br>      memory             = number<br>    }))<br>  }))</pre> | `{}` | no |
