...
| <a name="input_batch_command"></a> [batch\_command](#input\_batch\_command) | n/a | `string` | `"/var/www/html/vendor/bin/laminas --container=/var/www/html/config/container-cli.php"` | no |
...
| <a name="input_jobs"></a> [jobs](#input\_jobs) | The batch job defintion settings | <pre>map(object({<br>    job_name = string<br>    argument = string<br>    repository = string<br>    version    = string<br>    memory   = string<br>    cpu      = string<br>    batch_environment = string<br>    batch_subnets = list(string)<br>    batch_security_groups = list(string)<br>  }))</pre> | `{}` | no |
