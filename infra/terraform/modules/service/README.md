...
| <a name="input_batch_log_retention_days"></a> [batch\_log\_retention\_days](#input\_batch\_log\_retention\_days) | CloudWatch Logs retention in days for the per-batch-job log groups under /aws/batch/vol-app-<env>-<jobname>. Must be a CloudWatch enum value as above. | `number` | `1` | no |
