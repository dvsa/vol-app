...
| <a name="input_jobs"></a> [jobs](#input\_jobs) | The application command to run | <pre>map(object({<br>    jobs = list(string) <br>      command = string<br>      image = string<br>      memory = number<br>      security_group_ids = list(string)<br>      subnets = list(string)      <br>    }))</pre> | n/a | yes |
