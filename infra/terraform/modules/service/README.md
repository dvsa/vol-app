...
| <a name="input_jobs"></a> [jobs](#input\_jobs) | The application command to run | <pre>map(object({<br>    command       = list(string)<br>    image         = string<br>    memory        = string<br>    create_role   = bool<br>    create_queue  = bool<br>  }))</pre> | `{}` | no |
