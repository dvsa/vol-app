...
| <a name="module_batch_environment"></a> [batch\_environment](#module\_batch\_environment) | terraform-aws-modules/batch/aws | ~> 2.0.0 |
...
| <a name="input_job_definitions"></a> [job\_definitions](#input\_job\_definitions) | The application command to run | <pre>map(object({<br>    job_name      = string<br>    command       = list(string)<br>    image         = string<br>    memory        = string<br>  }))</pre> | `{}` | no |
