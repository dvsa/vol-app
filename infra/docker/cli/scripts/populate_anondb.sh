...
# IMPORTANT:
# This MUST be the *Aurora cluster identifier*, not a reader/writer instance
prod_cluster_id="${DBCLUSTER_ID}"
...
cleanup() {
    ...
    if aws rds describe-db-instances \
        ...
        log "Deleting temporary DB instance: $tmp_instance_id"
    ...
    if aws rds describe-db-clusters \
        ...
        log "Deleting temporary DB cluster: $tmp_cluster_id"
...
###############################################
# 1. CREATE SNAPSHOT FROM PROD AURORA CLUSTER
###############################################
log "Creating Aurora cluster snapshot: $snapshot_id"
aws rds create-db-cluster-snapshot \
    --db-cluster-snapshot-identifier "$snapshot_id" \
    --db-cluster-identifier "$prod_cluster_id" \
    --region "$region" >/dev/null
...
log "Waiting for temporary cluster to be available..."
...
source ./s3assume.sh \
    ...
    "$nonprod_assume_external_id"

log "Uploading anonymised dumps to S3"
aws s3 cp \
    "$anondb_dump_dir" \
    s3://devapp-olcs-pri-olcs-deploy-s3/anondata/ \
    --recursive \
    --exclude "*" \
    --include "olcs-db*anon-${env}-${DATE}.*"
