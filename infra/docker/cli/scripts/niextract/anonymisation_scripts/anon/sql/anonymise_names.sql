...
ALTER TABLE name_seed ADD COLUMN id INT AUTO_INCREMENT PRIMARY KEY;

UPDATE event_history e
LEFT JOIN name_seed f ON (f.id = 1 + (e.id % @name_seed_count))
LEFT JOIN name_seed l ON (l.id = 1 + ((e.id + 7) % @name_seed_count))
SET e.change_made_by = IF(e.change_made_by IS NOT NULL, CONCAT(f.forename, ' ', l.family_name), e.change_made_by)
WHERE e.change_made_by IS NOT NULL;
