<?php
    ...
    // No direct access allowed - redirect to index
    if (empty($referer)) {
    ...
    // Check referrer ends with specified string from config
    $needleLength = strlen($config['referrer_ends_with']);
    $refererHost = parse_url($_SERVER['HTTP_REFERER'], PHP_URL_HOST);
    if (!(0 === substr_compare($refererHost, $config['referrer_ends_with'], -$needleLength))) {
        header('Location: /', true, 302);
        exit;
    } // If config set to true, just do a 302 redirect to the target, otherwise continue and render the html below.
    else if ($config['headerRedirect'] === true) {
...
<head>
    ...
    <meta http-equiv="refresh" content="2;url=<?= $redirectString ?>"/>
