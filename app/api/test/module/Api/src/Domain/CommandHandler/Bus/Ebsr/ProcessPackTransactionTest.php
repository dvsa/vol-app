...
final class ProcessPackTransactionTest extends ProcessPackTestCase
{
     ...
     */
    #[\PHPUnit\Framework\Attributes\DataProvider('subsidyNamesProvider')]
    public function testHandleCommandNewApplication(array $names, array $expectedIds, array $expectedAreas): void
    {
        ...
        $subsidyArea = new TrafficAreaEntity();
        $subsidyArea->setId('F');
        $subsidyAuthority = new LocalAuthorityEntity();
        $subsidyAuthority->setId(87);
        $subsidyAuthority->setDescription('Milton Keynes Council');
        $subsidyAuthority->setTrafficArea($subsidyArea);
        $secondArea = (new TrafficAreaEntity())->setId('B');
        ...
        $parsedEbsrData = [
            ...
            'subsidised' => 'bs_in_part',
            'subsidyAuthorityNames' => $names,
            'subsidyDetail' => implode("\n", $names),
            'busNoticePeriod' => 1,
        ...
        $extraProcessedEbsrData = [
            'subsidyLocalAuthorities' => new ArrayCollection(array_map(static fn ($id) => $subsidyAuthorities[$id], $expectedIds)),
            'subsidyTrafficAreas' => new ArrayCollection(array_map(static fn ($id) => $subsidyAreas[$id], $expectedAreas)),
            'subsidised' => $this->refData['bs_in_part'],
            'trafficAreas' => $this->trafficAreas($parsedTrafficAreas, $trafficArea1, $trafficArea2, $trafficArea3),
            'naptanAuthorities' => $this->naptan($naptanCodes),
            'busNoticePeriod' => $this->references[BusNoticePeriodEntity::class][1],
        ...
        $this->assertSame($expectedIds, $savedBusReg->getSubsidyLocalAuthorities()->map(static fn ($la) => $la->getId())->toArray());
        $this->assertSame($expectedAreas, $savedBusReg->getSubsidyTrafficAreas()->map(static fn ($ta) => $ta->getId())->toArray());
        $this->assertSame(implode("\n", $names), $savedBusReg->getSubsidyDetail());
        $this->assertSame($extraProcessedEbsrData['trafficAreas']->toArray(), $savedBusReg->getTrafficAreas()->toArray());
        $this->assertSame($extraProcessedEbsrData['localAuthoritys']->toArray(), $savedBusReg->getLocalAuthoritys()->toArray());
    }
    ...
    public static function subsidyNamesProvider(): array
    {
        return [
            'exact' => [['Milton Keynes Council'], [87], ['F']],
            'mismatch' => [['CPCA'], [], []],
            'mixed' => [['Milton Keynes Council', 'CPCA'], [87], ['F']],
