...
/**
 * Cases SubmissionSection Controller
 *
 ...
 * @author Shaun Lizzio <shaun.lizzio@valtech.co.uk>
 */
class SubmissionSectionController extends OlcsController\CrudAbstract
{
     ...
     * @var string
     */
    protected $formName = 'SubmissionSection';
    ...
    public function processLoad($data)
    {
        ...
        $dataSnapshot = json_decode($data['dataSnapshot'], true);

        $data = $this->callParentProcessLoad($data);
        $data['fields']['submission'] = $this->params()->fromRoute('submission');
        $data['fields']['submissionSection'] = $sectionId;
        $key = array_search($sectionId, $dataSnapshot);
var_dump($dataSnapshot);
        echo "KEY-> $key";
        $data['fields']['sectionData'] = isset($dataSnapshot[$key]['data']) ? $dataSnapshot[$key]['data'] : '';
        return $data;
    }
