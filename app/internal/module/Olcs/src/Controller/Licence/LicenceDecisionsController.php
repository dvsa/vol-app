...
class LicenceDecisionsController extends AbstractController
{
    ...
    /**
     * IMO this action should be responsible for perfoming the active check logic and should then forward
     * onto the
     *
    ...
    public function activeLicenceCheckAction()
    {
        ...
        $forwardTo = $this->params()->fromRoute('forwardTo');
        ...
        if (!$licenceStatusHelper->isLicenceCurtailable($licenceId)) {
            if ($this->request->isPost()) {
                ...
                if ($this->isButtonPressed('cancel')) {
                    return $this->redirectToRoute('licence', array('licence' => $licenceId));
                ...
                $form->setAttribute(
                    'action',
                    $this->getUrlFromRoute('licence/curtail-licence', array('licence' => $licenceId)
                ));
                ...
                if (isset($postData['licence-decision-curtail']['form-actions']['curtailSave'])) {
                    ...
                    if ($form->isValid()) {
                        $licenceStatusEntityService->createStatusForLicence($licenceId, array(
                            'data' => array(
