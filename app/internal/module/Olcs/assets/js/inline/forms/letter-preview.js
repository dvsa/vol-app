OLCS.ready(function () {
  ...
  $(
    "input[name='sections[]'], input[name='appendices[]'], input[name='letterSections[]'], input[name='todos[]']",
  ).on("change", function () {
    var anyChecked =
      $("input[name='sections[]']:checked").length > 0 ||
      $("input[name='appendices[]']:checked").length > 0 ||
      $("input[name='letterSections[]']:checked").length > 0 ||
      $("input[name='todos[]']:checked").length > 0;
  ...
  $("#continue-to-editor").on("click", function (e) {
    ...
    // Collect selected to-do checkboxes
    var selectedTodos = [];
    ...
    if (
      ...
      var errorHtml =
        ...
        "<li>Please select at least one item to edit</li>" +
        "</ul></div></div>";
    ...
    // Build URL with everything that was ticked
    ...
    selectedTodos.forEach(function (todoId) {
      url += "&todos[]=" + encodeURIComponent(todoId);
    });
