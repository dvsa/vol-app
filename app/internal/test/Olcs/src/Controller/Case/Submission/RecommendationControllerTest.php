...
class RecommendationControllerTest extends AbstractHttpControllerTestCase
{
    ...
    public function testProcessSave($isOk)
    {
        ...
        $mockedRecommendationBusinessService = $this->getMock('stdClass', ['process'], array(), '', false, false);
        $mockedRecommendationBusinessService->expects($this->once())->method('process')
            ->with(
                [
                    'id' => $id,
                    'data' => $data['fields'],
        ...
        $mockedBusinessServiceManager->expects($this->once())->method('get')
            ->with('Cases\Submission\Recommendation')->will($this->returnValue($mockedRecommendationBusinessService));
