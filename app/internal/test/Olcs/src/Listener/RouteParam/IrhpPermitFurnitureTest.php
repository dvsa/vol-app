...
class IrhpPermitFurnitureTest extends TestCase
{
    ...
    public function testOnIrhpPermit()
    {
        ...
        $mockPlaceholder = m::mock()
            ...
            ->andReturn(
                m::mock()->shouldReceive('set')->once()
                    ->with('Foo ltd, Permit Application - ECMT Annual')
                    ->getMock()
            )
            ->getMock();
        ...
        $mockSidebarNavigation
            ...
            ->shouldReceive('findOneBy')->once()->with('id', 'irhp-permit-decisions-revive-from-unsuccessful')->andReturn(
                m::mock()->shouldReceive('setVisible')->once()->with(false)->getMock()
            )->getMock();
    ...
    public function testOnIrhpPermitBilateral()
    {
        $irhpPermitId = 2;
        $irhpPermit = [
            'id' => $irhpPermitId,
